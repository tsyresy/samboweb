...
export function Members() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-semibold text-ink">Annuaire des membres</h1>
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        <input
          ...
          className="w-full max-w-sm rounded-full px-4 py-2 text-sm field"
        />
        <select
          value={category}
          onChange={(e) => setCategory(e.target.value as MembershipCategory | '')}
          className="rounded-full px-4 py-2 text-sm field"
        >
      ...
      {loading ? (
        ...
        <p className="mt-8 text-ink-subtle">Aucun membre trouvé.</p>
      ) : (
        <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((m) => (
            <div
              key={m.id}
              className="flex items-center gap-4 rounded-2xl border border-line glass p-4"
            >
              {m.photo_url ? (
                <img src={m.photo_url} alt="" className="h-14 w-14 rounded-full object-cover" />
              ) : (
                <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-white/10 text-lg font-semibold text-accent">
                  {fullName(m).charAt(0)}
                </div>
              )}
              <div className="min-w-0">
                <p className="truncate font-medium text-ink">{fullName(m)}</p>
                {m.nickname && <p className="truncate text-xs text-ink-subtle">« {m.nickname} »</p>}
                <p className="text-xs text-ink-subtle">
                  {positions[m.id] ?? CATEGORY_LABELS[m.category]}
                </p>
                {m.phone && <p className="mt-1 text-xs text-ink-muted">{m.phone}</p>}
                {m.email && <p className="truncate text-xs text-ink-muted">{m.email}</p>}
              </div>
            </div>
          ))}
        </div>
